...
<form action="{{ route('modell.store') }}" method="POST">
    ...
    <select id="hajtasID" name="hajtasID" required>
        ...
        @foreach($hajtas as $h)
            <option value="{{ $h->id }}" @selected(old('hajtasID') == $h->id)>
                {{ $h->nev }}
            </option>
        @endforeach
    </select>
    ...
    <select id="uzemanyagID" name="uzemanyagID" required>
        ...
        @foreach($uzemanyag as $u)
            <option value="{{ $u->id }}" @selected(old('uzemanyagID') == $u->id)>
                {{ $u->nev }}
            </option>
        @endforeach
    </select>
